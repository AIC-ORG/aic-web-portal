...
const LiveRoomIndex: FC<Props> = ({ stream }) => {
  ...
  return (
    <div className="w-full text-white min-h-screen h-full flex flex-col">
      {user ? (
        <div className="flex mt-6 w-full lg:flex-row flex-col gap-3">
          <StreamPlayer webcamRef={webcamRef} stream={stream} remoteVideoRef={remoteVideoRef} />
          {/* Viewer-specific or Artist-specific UI */}
          <div className="chat-box rounded-xl border-bg-african bg-black flex min-h-[20vh] flex-col justify-between overflow-hidden border lg:w-1/3 min-w-[300px]">
            <div className="flex w-full bg-dark-brownish text-white px-6 py-3">
              <span>Live Chat</span>
            </div>
            ...
            <div className="flex p-2">
              ...
              >
                ...
                />
                <input
                  className=" h-full bg-transparent outline-none focus-within:border-black duration-300 w-full py-2 my-2 ml-2"
                  type="text"
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                />
                <button
                  className="bg-dark-brownish hover:bg-bg-african duration-300 h-full text-white py-2 px-4"
                  type="submit"
                >
                  Send
                </button>
        ...
        <div className="flex flex-col gap-y-3 justify-center items-center h-full">
          <p className="text-2xl text-white">Please login to view this page</p>
          <Link
            href={{
              pathname: '/login',
              query: { redirect: `/live/${stream.id}` },
            }}
          >
            <button className="bg-bg-african mt-4 text-center w-44 h-11 mx-auto rounded text-white text-xl font-bold">
              Login
            </button>
          </Link>
        </div>
